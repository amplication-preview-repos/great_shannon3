/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SportsWhereInput } from "./SportsWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { SportsOrderByInput } from "./SportsOrderByInput";

@ArgsType()
class SportsFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => SportsWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => SportsWhereInput, { nullable: true })
  @Type(() => SportsWhereInput)
  where?: SportsWhereInput;

  @ApiProperty({
    required: false,
    type: [SportsOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [SportsOrderByInput], { nullable: true })
  @Type(() => SportsOrderByInput)
  orderBy?: Array<SportsOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { SportsFindManyArgs as SportsFindManyArgs };
